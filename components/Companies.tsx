...
export default function Companies() {
  ...
  return (
    <section id="benefits" className="py-16 sm:py-20 md:py-28 bg-white">
      <div>

        {/* Header */}
        <div className="mb-12 sm:mb-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
            <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-light leading-[1.1] text-[#1a1a1a] max-w-3xl">
              Warum mit KLAIMLY arbeiten?
            </h2>

            {/* Navigation Buttons */}
            <div className="flex gap-3 sm:gap-4 flex-shrink-0 sm:ml-8">
              <button
                onClick={scrollLeft}
                className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-gradient-to-br from-[#7A866C] to-[#93A084] hover:from-[#6B7760] hover:to-[#849175] transition-all duration-300 flex items-center justify-center shadow-lg hover:shadow-xl transform hover:scale-105"
                aria-label="Previous"
              >
          ...
          >
            <div className="flex gap-4 sm:gap-6">
              {companies.map((company, index) => (
                <div
                  key={index}
                  className={`flex-shrink-0 w-[85vw] sm:w-[70vw] md:w-[45vw] lg:w-[30vw] max-w-[400px] ${
                    index === 0 ? 'ml-4 sm:ml-6 lg:ml-[max((100vw-1280px)/2+2rem,2rem)]' : ''
                  } ${
                    index === companies.length - 1 ? 'mr-4 sm:mr-6 lg:mr-8' : ''
                  }`}>
                  <div className="relative rounded-2xl sm:rounded-3xl overflow-hidden group cursor-pointer h-[380px] sm:h-[440px] md:h-[550px] bg-gradient-to-br from-[#f8f8f8] to-[#f0f0f0]">
                    {/* Content */}
                    <div className="absolute inset-0 p-6 sm:p-8 md:p-10 flex flex-col justify-between">
                      <div>
                        ...
                        <h3 className="text-[#1a1a1a] text-xl sm:text-2xl md:text-3xl font-light leading-[1.2] mt-3 mb-4 sm:mb-6">
                          {company.name}
                        </h3>
                      </div>
